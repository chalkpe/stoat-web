/**
 * Regex for matching multi-line spoilers
 */
const RE_MULTILINE_SPOILER = /\[\[([\s\S]*?)\]\]/gm;
...
export function sanitise(content: string) {
  return (
    content
      // Replace newlines inside spoiler tags with special character
      // so they are preserved through markdown parsing
      .replace(RE_MULTILINE_SPOILER, (match) => match.replace(/\n/g, "\uF801"))
